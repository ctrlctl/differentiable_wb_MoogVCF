...
class MoogVCFRFModelCell(nn.Module):
    def __init__(self, r, f, gain, sample_rate):
        ...
        self.gf = nn.Parameter(torch.FloatTensor([1.6004]), requires_grad=True)#1.6762
        self.gr = nn.Parameter(torch.FloatTensor([1.2305]), requires_grad=True)#1.0
        self.gain = nn.Parameter(torch.FloatTensor([3.2148]), requires_grad=True)#1.6762
    ...
    def update_matrices(self, previous_state, previous_input):
        ...
        self.k_b_div_two = self.k * self.B / 2
    ...
    def forward(self, u_n, x_n1, u_n1, batch_size, device):
        ...
        for i in range(0, batch_size):
            ...
            p_n[i] = x_n1[i] + self.k_A_div_two.matmul(x_n1_th) + self.k_b_div_two.matmul(u_n[i] + u_n1[i])
            x_temp = x_n1[i]
            ...
            for j in range(0, max_iter):
                ...
                g = p_n[i] + self.k_A_div_two.matmul(x_t_th) - x_temp

                # Calculate J
                x_t_one_m_p_th = torch.zeros_like(x_n[i])
                for m in range(0, x_n.shape[1]):
                    x_t_one_m_p_th[m] = 1 - pow(math.tanh(x_temp[m]), 2)
                J = self.k_A_div_two.matmul(x_t_one_m_p_th) - self.E
                deltax = g / J
