...
class MoogVCFRFModelCell(nn.Module):
    def __init__(self, r,  If, C, VT, gain, sample_rate):
        ...
        self.gIf = nn.Parameter(torch.FloatTensor([1.0915]), requires_grad=True)#1.0462 1.2594
        self.gC = nn.Parameter(torch.FloatTensor([1.0857]), requires_grad=True)#1.0552 0.7577
        self.gVT = nn.Parameter(torch.FloatTensor([1.0857]), requires_grad=True)#1.0552 0.7577
        self.gr = nn.Parameter(torch.FloatTensor([1.1076]), requires_grad=True)#1.0502 0.8571
        self.gain = nn.Parameter(torch.FloatTensor([3.8975]), requires_grad=True)# 0.4914
        self.alpha = nn.Parameter(torch.FloatTensor([1.1498]), requires_grad=True) #1.0 alpha = 1, traperzoidal rule
